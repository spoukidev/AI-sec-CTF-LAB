...
Move allowed numeric features across a synthetic classifier boundary.

## Vulnerability

A brittle feature boundary is exposed to attacker-influenced inputs.
...
~~~mermaid
flowchart LR
    NumericSample --> Classifier --> Prediction
~~~
...
A brittle feature boundary is exposed to attacker-influenced inputs. The deliberately vulnerable engine makes this behavior deterministic so the lesson does not depend on a commercial model.
...
Threat-model mutable features, test adversarially, monitor drift, and combine robust signals.
