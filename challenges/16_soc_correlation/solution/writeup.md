...
Make unrelated alerts share one incident.

## Vulnerability

A single matching field is treated as sufficient causal evidence.
...
A single matching field is treated as sufficient causal evidence. The deliberately vulnerable engine makes this behavior deterministic so the lesson does not depend on a commercial model.
...
Use asset identity, technique, session, direction, and confidence with analyst-visible evidence.
