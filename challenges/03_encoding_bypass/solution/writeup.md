...
Express the request without blocked keywords.

## Vulnerability

A lexical denylist cannot recognize equivalent intent.
...
A lexical denylist cannot recognize equivalent intent. The deliberately vulnerable engine makes this behavior deterministic so the lesson does not depend on a commercial model.
...
Use authorization and output controls; filters may be defense-in-depth only.
