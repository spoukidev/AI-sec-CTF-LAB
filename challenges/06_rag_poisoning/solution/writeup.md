...
Make an uploaded poisoned chunk rank first.

## Vulnerability

Relevance scoring ignores origin, approval state, and instruction-like content.
...
~~~mermaid
flowchart LR
    Upload --> Retriever --> PoisonedChunk --> MockLLM
~~~
...
Relevance scoring ignores origin, approval state, and instruction-like content. The deliberately vulnerable engine makes this behavior deterministic so the lesson does not depend on a commercial model.
...
Require ingestion approval, provenance, trust-aware ranking, and post-retrieval filtering.
