...
Access a synthetic admin report while acting as analyst.

## Vulnerability

A privileged agent becomes a deputy because the tool trusts its decision.
...
A privileged agent becomes a deputy because the tool trusts its decision. The deliberately vulnerable engine makes this behavior deterministic so the lesson does not depend on a commercial model.
...
Bind scoped credentials to users and enforce role checks inside the tool boundary.
